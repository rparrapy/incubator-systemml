...
public abstract class ArithmeticBinaryFLInstruction extends BinaryFLInstruction {
    ...
    public ArithmeticBinaryFLInstruction(Operator op, CPOperand in1, CPOperand in2, CPOperand out, String opcode,
                                         String istr) {
        super(op, in1, in2, out, opcode, istr);
        _fltype = FLINSTRUCTION_TYPE.ArithmeticBinary;
    }
    ...
    public static ArithmeticBinaryFLInstruction parseInstruction(String str)
        ...
        if (str.startsWith("FLINK" + Lop.OPERAND_DELIMITOR + "map")) {
        ...
        if (dt1 == DataType.MATRIX || dt2 == DataType.MATRIX) {
            if (dt1 == DataType.MATRIX && dt2 == DataType.MATRIX) {
                if (isBroadcast)
                    return new MatrixBVectorArithmeticFLInstruction(operator, in1, in2, out, vtype, opcode, str);
                else
                    return new MatrixMatrixArithmeticFLInstruction(operator, in1, in2, out, opcode, str);
            } else
                return new MatrixScalarArithmeticFLInstruction(operator, in1, in2, out, opcode, str);
        }
